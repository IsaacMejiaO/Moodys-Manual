...
- DCF Valuation (SEC-primary beta, two-stage model, sensitivity tables)
...
Fixes vs. prior version:
  ...
  - DCF page added: render_dcf() reads from the same session-state cache
    as all other pages — zero redundant API calls.
"""
import sys
import os
import logging
...
def fetch_company_data_unified(ticker: str, cik: str = None):
    ...
    This ensures consistency across all pages (Screener, Tearsheet, Multiples, Ratios, DCF).
...
show_ticker_selector = current_page in {
...
# =========================================================
    ...
    if not ticker:
        ...
        # Pass cached data and summary so the DCF page never double-fetches.
        # If the user navigated here from Financials or Tearsheet the data
        # is already in session state — render_dcf() will use it directly.
