...
#
# capital_iq_style_peer_finder.py imports get_peer_override from this module,
# so the canonical registry lives here. aggregation.py re-exports the same
# functions by importing from here, keeping a single source of truth.
#
...
def get_peers_by_sic_deterministic(sic_code: str, max_results: int = 200) -> List[str]:
    """
    Stub for deterministic SIC-based peer expansion.
    Returns an empty list; the full implementation lives in
    capital_iq_style_peer_finder.py to avoid circular imports.
    This stub ensures peer_finder.py is self-contained.
    """
    return []
